...
public  class SectionEditorAdapter extends BaseExpandableListAdapter
  ...
  private ExpandableListMultiSelectAdapter adapterWrapper;
  ...
  private void initItemPositionMap() {
    ...
    for (SectionModel group : groups) {
      for (SectionModel.Pair item : group.getItems()) {
  ...
  public View getGroupView(
    ...
    if (convertView == null) {
      convertView =
          LayoutInflater.from(context).inflate(R.layout.doublelist_item_group, parent, false);
  ...
  public View getChildView(
    ...
    if (convertView == null) {
      convertView =
          LayoutInflater.from(context).inflate(R.layout.doublelist_item_item, parent, false);
  ...
  public void onTranslate(boolean enable_llm) {
    ...
    dialog.show();
    int[] totalTasks = {0};
    List<Runnable> tasks = new ArrayList<>(); // 存储所有待执行任务
        ...
        if (multiSelectManager == null || !multiSelectManager.isMultiSelectMode() || multiSelectManager.isSelected(getPosition(item))) {
          ...
          var translateCallBack = new Translator.TranslateCallBack() {
            ...
            public void onSuccess(
              ...
              Timber.d("translation:%s", translation);

              if (SettingsFragment.TranslateMode == SettingsFragment.ADD) {
                var lang_pairs = item.getLang_pairs();
                lang_pairs.put(targetLanguage, translation);
              } else {
                item.setOri_val(translation);
              }
          ...
          if(enable_llm){
            Translator.LLM_translate(item.getOri_val(),
                    AppSettings.CurrentFromLanguage,
                    AppSettings.CurrentTargetLanguage,
                    translateCallBack);
          }else {
  ...
  // 装饰器负责管理多选模式下的视觉效果
  public class DoubleListDecorator {
